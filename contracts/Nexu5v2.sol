...
contract Nexu5 is ERC721A, Ownable, ReentrancyGuard {
  ...
  function founderMint(uint256 quantity) external payable callerIsUser {
    ...
    //uint256 totalCost = getFounderPrice(); // @Proteu5: Unused For Nexu5 - Founders = Claim
  ...
  function allowlistMint() external payable callerIsUser {
    ...
    require(msg.value >= price, "add funds");
    ...
    callerIsUser
  {
    ...
    require(
      isPublicSaleOn(publicPrice), // @Proteu5: added !=0
      "public sale != true"
    );
 ...
 function isWhistlistSaleOn() public view returns (uint64 mintListTrueFalse) {
    if(uint256(saleConfig.mintlistPrice) != 0)
    {
        mintListTrueFalse = 1;
        return mintListTrueFalse;
    }
    else if (uint256(saleConfig.mintlistPrice) == 0)
    {
      mintListTrueFalse = 0;
      return mintListTrueFalse;
    }
